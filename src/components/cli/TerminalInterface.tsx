...
import { USERNAME, HOSTNAME, PROMPT_SYMBOL, VALID_THEMES as VALID_THEMES_CONST, EASTER_EGG_IDS as EASTER_EGG_IDS_CONST } from './constants';
import { useCliTheme, type Theme as CliThemeType } from './hooks/useCliTheme';
...
const VALID_RESUME_SECTIONS = ['education', 'skills', 'projects', 'certifications', 'articles', 'interests'];
...
export default function TerminalInterface() {
  ...
  const [resumeSections, setResumeSections] = useState<string[]>(VALID_RESUME_SECTIONS);
  ...
  const processCommand = async (command: string): Promise<React.ReactNode | { openModal: 'resume', sections?: string[] } > => {
    ...
    switch (cmd) {
      ...
      case 'certifications':
      case 'certs':
        if (allFlag) { // Even if 'all' is always shown now, keep easter egg for -a
            triggerEasterEgg(EASTER_EGG_IDS.CERTS_ALL);
        }
        return <CertsOutput mode={'all'} />; // Always show all certs in CLI
      case 'gaming':
      ...
      case 'pdf':
      case 'cv':
        const requestedSections = args.filter(arg => VALID_RESUME_SECTIONS.includes(arg.toLowerCase()));
        const sectionsForModal = requestedSections.length > 0 ? requestedSections : VALID_RESUME_SECTIONS;
 ...
 const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
  ...
  return (
    <>
      <div className="flex flex-col h-full p-2 md:p-4 bg-background text-sm md:text-base"
        onClick={handleContainerClick}
      >
        <ScrollArea className="flex-grow cli-output-area overflow-y-auto">
          <div>
            {history.map((line) => (<div key={line.id} className={`text-sm md:text-base mb-1 ${line.type === 'input' ? 'text-primary' : line.type === 'error' ? 'text-destructive' : line.type === 'system' ? 'text-muted-foreground' : 'text-foreground'}`}>
              {typeof line.content === 'string' ? <pre className="whitespace-pre-wrap">{line.content}</pre> : line.content}
            </div>))} 
            <div ref={endOfHistoryRef} />
          </div>
        </ScrollArea>
        <form onSubmit={handleCommandSubmit} className="mt-2 flex flex-row flex-wrap items-baseline flex-shrink-0 mb-0 pb-0">
          ...
          <div className="relative flex flex-grow items-baseline min-w-[calc(50%+1rem)] sm:min-w-[200px]">
            <span className="cli-input-display whitespace-pre" style={{whiteSpace: 'pre', fontSize: '0.875rem'}}>
              {inputValue}
