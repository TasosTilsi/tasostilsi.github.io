...
// ---------------------------------------------------------------------------
// buildProjectCalendar — year-grid bars calendar geometry (D-07, UI-SPEC §5)
// ---------------------------------------------------------------------------
...
export interface ProjectCalendarData {
  ...
  endYear: number;
  yearColumns: number[];
  rows: ProjectCalendarRow[];
}

/** A date string that is EXACTLY a year ('2026') — year-only precision (OQ-1). */
const YEAR_ONLY_PATTERN = /^\s*(?:19|20)\d{2}\s*$/;

/**
 * Year-grid calendar geometry (D-07, UI-SPEC §5): project ship dates →
 * percentage positions on a shared year-column axis. Axis = January of the
 * minimum data year through December of `now.getFullYear()` — full-year
 * columns (not min-ship-month start) so each year is an exact 1/N block, a
 * year GRID per the user's framing; `now` INJECTABLE for tests (the SSG
 * default freezes the axis until rebuild, OQ-7 precedent). Rows render in
 * JSON order — NO sorting of any kind (R-4/E-6); name/date are copied
 * VERBATIM as stored (the parser touches geometry only). A 'Month YYYY'
 * date places a ONE-month-cell bar (D-07: single ship month); a year-only
 * date ('2026') spans its FULL year — 12 months, no month invented (OQ-1:
 * the precision the data carries; recorded override of the UI-SPEC §5.4
 * no-bar reading, see the suite header); any other date ('Ongoing',
 * malformed) yields null geometry — the row is kept, the bar omitted,
 * never invented (E-1). No parseable year anywhere → startYear falls back
 * to the now year and every row goes null (E-2 — the renderer then hides).
 * yearColumns: startYear..endYear inclusive.
 */
export function buildProjectCalendar(
  ...
  const parsed = projects.map((project) => {
    ...
    if (!date) return null;
    const monthYear = parseMonthYear(date);
    if (monthYear) {
      return { start: monthIndex(monthYear.year, monthYear.month), months: 1 };
    }
    const yearOnly = date.match(YEAR_ONLY_PATTERN);
    if (yearOnly) {
      return { start: Number(yearOnly[0]) * 12, months: 12 };
    }
    return null;
  });
  ...
  for (const entry of parsed) {
    if (entry) {
      startYear = Math.min(startYear, Math.floor(entry.start / 12));
    }
  }
  if (startYear === Number.POSITIVE_INFINITY) {
    startYear = endYear;
  }
  ...
  const rows: ProjectCalendarRow[] = projects.map((project, index) => {
    const base = { index, name: project.name, date: project.date ?? '' };
    const entry = parsed[index];
    if (!entry || totalMonths <= 0) {
      return { ...base, leftPct: null, widthPct: null };
    }
    return {
      ...base,
      leftPct: ((entry.start - axisStart) / totalMonths) * 100,
      widthPct: (entry.months / totalMonths) * 100,
    };
  });
  ...
  return { startYear, endYear, yearColumns, rows };
}