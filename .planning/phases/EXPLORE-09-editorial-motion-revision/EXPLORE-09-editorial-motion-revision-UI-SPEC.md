...
- Education markers (discretion, pinned default): **hollow dot** — same footprint, `rounded-full border border-chart-2 bg-transparent` — plus the plain year label. No glyph, no text suffix (mono label stays clean). (UNRESOLVED U-8: a small degree glyph instead of the hollow dot.)
...
- **Body inside the pinned shell** (top → bottom): `ProjectStatTiles` (unchanged, stays **above** the rows) → **rows viewport** (`flex-1 relative overflow-hidden`, descendant of the sticky shell — sticky-safe, does not sit on the sticky ancestor chain) → `TerminalPointer command="projects --all"` (last, always visible while pinned — the Experience pointer precedent, experience-section.tsx:250) → **compact list** (`md:hidden`, the existing card grid byte-identical) between tiles and pointer in DOM is NOT used — the compact list sits after the rows viewport, `md:hidden`; pointer is last in both tiers.
