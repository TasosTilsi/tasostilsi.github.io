...
| 2 | `AVAILABLE_COMMANDS` = 38 entries (constants.ts:55-62); Tab autocomplete (single/multi-match) and fuzzy "Did you mean" suggestions both derive from it. No command starts with `ex…` — `explore` collides with nothing. | `constants.ts:55-62`, `TerminalInterface.tsx:299-304, 444-461` |
| 3 | `processCommand` returns `ReactNode` (→ pushed as `output` history line) or a sentinel object (`{ openModal: "resume" }` → handled in `executeCommand`). The sentinel path is the established precedent for side-effecting commands. | `TerminalInterface.tsx:129-133, 352-386, 399-403` |
...
| 5 | The CLI's sr-only live region announces the **string** content of the last `output` line only; node content falls back to the generic `"Command executed"`. | `TerminalInterface.tsx:515-521` |
...
| 9 | **CORRECTION / supersession:** Phase-1 pinned "drawer RIGHTMOST" (re-affirmed phase-4 §5). **D-03 (newer, explicit) supersedes it**: the Terminal link becomes the new rightmost control. The stale code comment at `explore-header.tsx:4-11` must be updated (comment-only edit) so no implementer reverts the order. | `explore-header.tsx:4-11`, phase-4 UI-SPEC §5, CONTEXT D-03 |
...
| 17 | Wizard finish-card link: label `"Open the terminal →"`, `href: "/"`, rendered as `<Link … className="text-accent">` — **unchanged this phase**. | `constants.ts:89-94`, `explore-tour.tsx:435-437` |
...
- Perceptibility: the chrome line should be on screen ≥ ~0.8s before the route swaps — default **800ms delay before navigation**, following the resume-loading precedent (§0#4). (UNRESOLVED → §12: zero-delay accepted if the planner prefers, but the default is the delay.)
...
- The sr-only live region announces node-content lines generically as `"Command executed"` (§0#5) — accepted. The route change itself is the meaningful announcement (page title / landmarks change). Do not add a second string line just for SRs (one chrome line only, D-02).
...
| `explore` command chrome line | bracket line in history (`system`) | — | — | sr log: generic "Command executed" (§3.3) | n/a | visible ≥ ~0.8s (default delay), then route swap | n/a |
...
| (UNRESOLVED) | Pre-navigation delay for the chrome line | 800ms (resume precedent, §0#4); zero-delay accepted |
...
2. No new dependencies (lucide `Terminal`, `next/link`, `useRouter` all already present).
