...
| 6 | 768 | / | P+M | ASCII banner visible (`sm:block` from 640px) · CLI layout invariants (as row 5) · [M] visual pass | P parts pass (plan-03 sweep suite); M pending user final pass | pass (P) · manual — user final pass (M) | tests/explore-sweep.test.mjs rows CLI@≥768 |
| 7 | 1440 | / | P+M | ASCII banner visible · CLI layout invariants (as row 5) · [M] visual pass | P parts pass (plan-03 sweep suite); M pending user final pass | pass (P) · manual — user final pass (M) | tests/explore-sweep.test.mjs rows CLI@≥768 |
| 8 | 1920 | / | P+M | ASCII banner visible · CLI layout invariants (as row 5) · [M] visual pass | P parts pass (plan-03 sweep suite); M pending user final pass | pass (P) · manual — user final pass (M) | tests/explore-sweep.test.mjs rows CLI@≥768 |

## Export-level (E) rows

_Filled in Task 2 after the wave's single `npm run build` (plans 01/02 do not build)._
