...
must_haves:
  truths:
    ...
    - "The merged panel carries, in UI-SPEC §3.2's pinned order — description → meta row → divider → all 9 contact channel rows → 'Full resume →' link LAST — under About's chart-1 accent and panel identity; the Full-resume link is the merged panel's last interactive element (§13 E-UAT-2) and no standalone Contact panel exists"
