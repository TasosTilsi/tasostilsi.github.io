...
<tasks>
  <task type="auto">
    ...
    <files>tests/projects-calendar.test.mjs, src/components/explore/viz-data.ts, src/components/explore/sections/projects-calendar.tsx, src/components/explore/sections/projects-section.tsx, src/app/globals.css</files>
    ...
    <acceptance_criteria>
      ...
      - projects-section.tsx composes ProjectsCalendar as the FIRST body child above the tiles wrapper and before the cards map; the cards JSX block is byte-unchanged (git diff scoped to the insertion)
      - globals.css carries a light-mode --chart-4 override beside the chart-2/3 block; a source test row asserts it
  ...
  <task type="auto">
    ...
    <verify>npm run typecheck && node --test tests/explore-visuals-skills.test.mjs && ! test -f src/components/explore/sections/skills-chart.tsx && ! test -f src/components/explore/sections/skills-treemap.tsx && ! grep -rn "recharts" src/components/explore/ && ! grep -q "techMentions" src/components/explore/viz-data.ts — checker W-3b fix: tests/explore-visuals.test.mjs is SCOPED here (node --test --test-name-pattern="^(?!.*export)" tests/explore-visuals.test.mjs) because its rewritten export-level rows read the stale pre-phase out/ until task 3's build regenerates it; the full unscoped visuals run belongs to task 3's final gate</verify>
    <acceptance_criteria>
