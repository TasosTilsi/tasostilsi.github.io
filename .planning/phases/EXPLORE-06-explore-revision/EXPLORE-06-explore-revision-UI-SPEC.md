...
4. **PROFESSIONAL EXPERIENCE** — the docx's 3 roles, reverse-chron data order, FULL quantified bullet list per role (no 3-bullet cap on /resume; the docx carries 6/3/4). **CORRECTED (B-1): the data actually carries FOUR `isTechRelated:true` entries — Chubb, Upstream Systems, Netcompany-Intrasoft AND Smartup PCC (positions 0–3).** The filter alone therefore yields 4 roles, not the docx's 3. Resolution: the filter stays, and the **data write flips Smartup PCC's `isTechRelated` to `false`** — Smartup PCC remains fully CLI-reachable (`experience --all`), the /resume+PDF role-selection contract is restored to exactly 3. This is a named data-write obligation (U-4 re-pinned), not executor improvisation.
...
6. **EDUCATION** — the docx's 2 degrees (MSc + BEng) in docx order; existing row anatomy (degree/duration/institution | location/focus) unchanged. **CORRECTED (B-2): the data actually carries THREE education entries — the third is "High School Degree" (1st General High School of Sykies).** Nothing is deleted (CLI-reachability invariant): the data write adds a **`featured: true` flag to MSc + BEng only**, and /resume + the PDF render featured entries only — the same mechanism the certifications section uses. The high-school row stays CLI-reachable via the `education` command.
...
| E-11b | **Featured project missing `link` (W-2 pin)** | The muted mono link-line atom (§7.3.5) is **omitted** for that project — name/date/description still render; never an empty line or placeholder URL |
...
| E-14 | Hardcoded 5/7-length chrome arrays | `PRIMARY_LABELS` → 6, `DIGIT_ACCENTS` → 4, step bodies → 4, welcome "five"→"four" — all pinned in §6; the sr-only "of 7" becomes derived. **+ (W-1): the `TourStep["id"]` union at `constants.ts:77` still enumerates `"contact"` — the data write/step re-mapping must shrink it to the 4 remaining sections (type-level consequence of the merge; add to this inventory)** |
...
| U-4 | /resume experience role-selection mechanism | Keep the existing `isTechRelated` filter **+ the data write flips Smartup PCC's `isTechRelated` to `false`** (B-1 correction: 4 entries are `true` today, not 3) — together they yield exactly the docx's 3 roles; the data write swaps the bullets. Smartup PCC stays CLI-reachable. |
