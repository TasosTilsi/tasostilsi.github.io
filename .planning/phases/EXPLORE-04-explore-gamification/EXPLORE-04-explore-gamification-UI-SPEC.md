...
1. **Cut-out hole element** (content steps only): absolutely-positioned transparent element sized to the measured panel rect **inflated 12px on all sides**, `rounded-md`, `box-shadow: 0 0 0 100vmax rgba(0,0,0,0.7)` (D-01 technique verbatim; **spread amended 100vw → 100vmax**, surfaced per RESEARCH R-1: `100vw` under-covers tall portrait viewports — at 375×812 a hole bottom above 437px leaves an unshaded strip; `100vmax` guarantees coverage in every direction with the same one-element technique, color, and opacity), `pointer-events: none`, `aria-hidden="true"`. The box-shadow IS the dim layer — one element, no SVG mask. The 12px ring shows undimmed page background around the panel (intended spotlight breathing ring; hole radius 2px vs panel radius 2px with a 12px ring — corner mismatch is imperceptible).
...
**IntersectionObserver marking path (B-2 resolution — D-06/SPEC EXPLORE-04c contract):** besides drawer anchors and wizard arrivals, **manual scroll-into-view marks sections visited** via `IntersectionObserver` with `root = document.querySelector('.explore-shell > main')` (the sole scroll container — never the window), **per-panel height-aware `threshold = min(0.5, (main.clientHeight × 0.5) / panelHeight)` computed at observer creation (R-2 amendment of the original fixed `0.5`: the threshold is a ratio against the TARGET's own bounding box, so a panel taller than 2× the visible root could never reach 0.5 and would never mark by scroll — the per-panel value preserves the ~50%-visible intent)**, one observer per panel (IO applies a single threshold list to all targets, so per-target thresholds require per-panel observers), all created in one mount-only effect. Semantics pinned: (a) a panel marks visited when its height-aware threshold is crossed — **once per panel per session, deduped in the store**; (b) the observers are created in a mount-only effect after hydration (SSG-safe — no server work), never re-created; (c) while the tour overlay is open, the observers keep running (wizard arrivals mark independently; no double-marking harm — dedupe covers it); (d) the drawer-open state does not pause the observers (the drawer is transient); (e) marking is idempotent and fires no toasts. A §12 source hook asserts the observer wiring exists with root/threshold pinned.
...
| E-3 | Drawer anchor click mid-tour | Closes drawer, scrolls (existing path), marks visited via the shared IntersectionObserver path (**OQ-3 resolution per RESEARCH R-3**: the drawer's anchor scroll is observed like any other scroll — drawer internals stay byte-untouched, **no `onSectionSelect` prop**); the tour does **NOT** follow to that section — the locked sequence is kept; cut-out re-aligns to the current step's panel at scrollend |
...
5. **Overlay DOM contract (source-level):** `z-40` on the wrapper; `pointer-events-none` wrapper + `pointer-events-auto` card; `box-shadow: 0 0 0 100vmax rgba(0,0,0,0.7)` on the hole (R-1 amendment — the spread is pinned `100vmax`, superseding this hook's original `100vw` string); `rounded-md` hole; overlay rendered as a child of the shell root (NOT portaled — no `createPortal` in tour files).
