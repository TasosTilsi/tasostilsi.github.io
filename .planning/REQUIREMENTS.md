...
- [ ] REV-04: The About and Contact panels merge into one panel on /explore, reclaiming the empty space the oversized About section left, with the panel grid rebalanced.
...
- [ ] REV-07: The Experience showcase is redesigned per the user's reference screenshot (layout description pending from the user — this requirement is scoped when the description arrives; the current text timeline + Gantt remain until then).
